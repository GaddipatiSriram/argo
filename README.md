...
Chart code lives in separate repos — this dir just has the Argo-layer
wiring (destinations, selectors, source paths).
...
│   ├── forge/                        ← ApplicationSets targeting role=forge
│   │   ├── cert-manager-appset.yaml
│   │   ├── external-secrets-appset.yaml
│   │   ├── ingress-nginx-appset.yaml
│   │   ├── keycloak-appset.yaml
│   │   ├── vault-appset.yaml
│   │   ├── forge-secret-sre.yaml     ← git repo creds
│   │   └── forge-secret-devops.yaml  ← git repo creds (argocd-devops)
│   ├── okd/                          ← per-cluster Applications on OKD itself
...
| **Argo Workflows** | workflow engine (DAG pipelines) | yes: `workflows.apps.mgmt-devops-okd.engatwork.com` | Argo Application (`argo/sre/okd/`) | ✅ |
...
| Service | Nature | UI | Install | Status |
|---|---|---|---|---|
| **Rook-Ceph operator** | Ceph daemon controller + CRDs | no (operator) | Argo Application (`argo/sre/mgmt-storage/rook-ceph-operator-app.yaml`), wave 0 | ✅ |
...
| **rook-ceph-external** | Ceph CSI client | no | Argo AppSet (Pattern A, `argo/sre/storage/`) | ✅ (Stage 3) |
...
Every ApplicationSet in `argo/sre/`, its selector pattern, what it
...
|---|---|---|---|---|---|---|---|
| **rook-ceph-external** | `argo/sre/storage/rook-ceph-external-appset.yaml` | A | `tier In [mgmt,dev]` AND `role NotIn [storage]` | mgmt-forge | mgmt-observability, mgmt-core, dev-* (once registered) | `storage/rook-ceph-external/chart/` | ✅ |
| **cert-manager** | `argo/sre/forge/cert-manager-appset.yaml` | B (should be A) | `role: forge` | mgmt-forge | (every workload cluster, after Pattern-A refactor) | `forge/cert-manager/` | ✅ |
| **ingress-nginx** | `argo/sre/forge/ingress-nginx-appset.yaml` | B (should be A) | `role: forge` | mgmt-forge | (every workload cluster, after Pattern-A refactor) | `forge/ingress-nginx/` | ✅ |
| **external-secrets** | `argo/sre/forge/external-secrets-appset.yaml` | B (should be A) | `role: forge` | mgmt-forge | (every workload cluster, after Pattern-A refactor) | `forge/external-secrets/` | ✅ |
| **vault** | `argo/sre/forge/vault-appset.yaml` | B | `role: forge` | mgmt-forge | mgmt-forge only (central service) | `forge/vault/` | ✅ |
| **keycloak** | `argo/sre/forge/keycloak-appset.yaml` | B | `role: forge` | mgmt-forge | mgmt-forge only (central service) | `forge/keycloak/` | ✅ |
...
|---|---|---|---|---|
| `rook-ceph-operator-mgmt-storage` | `argo/sre/mgmt-storage/rook-ceph-operator-app.yaml` | mgmt-storage | `storage/rook-ceph/operator/` | ✅ |
| `rook-ceph-cluster-mgmt-storage` | `argo/sre/mgmt-storage/rook-ceph-cluster-app.yaml` | mgmt-storage | `storage/rook-ceph/cluster/` | ✅ |
| `argo-workflows-okd` | `argo/sre/okd/argo-workflows-app.yaml` | OKD (in-cluster) | `forge/okd/argo-workflows/` | ✅ |
| `jenkins-okd` | `argo/sre/okd/jenkins-app.yaml` | OKD (in-cluster) | `forge/okd/jenkins/` | ✅ |
...
5. **Git repo credentials** Secrets applied
   (`argo/sre/forge/forge-secret-sre.yaml`,
   `argo/sre/forge/forge-secret-devops.yaml`).
...
4. **Register with argocd-sre**: apply `argo/sre/mgmt-storage/bootstrap-sa.yaml`
...
`argo/sre/mgmt-storage/README.md`.
...
2. Create `argo/sre/forge/<service>-appset.yaml` with
   `selector: { matchLabels: { role: forge } }`.
3. `kubectl apply -f argo/sre/forge/<service>-appset.yaml`.

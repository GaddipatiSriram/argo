...
`argo/sre/<cluster>/` or `argo/devops/` and this doc gets updated/closed.
...
Today ESO runs only on `mgmt-forge` (`argo/sre/forge/external-secrets-appset.yaml`
...
**File:** `argo/sre/okd/cluster-secret.yaml` — new. Mirrors the
...
**First — audit the operator-managed default.** `argo/README.md:489` notes an
...
**File:** `argo/sre/forge/external-secrets-appset.yaml` — the existing
